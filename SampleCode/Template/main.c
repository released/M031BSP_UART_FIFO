...
fifoBuf_t UART_InputBuffer;
#if defined (USE_UART_TX)
fifoBuf_t UART_OutputBuffer;
#endif
...
void FifoBufferSendDataPop(FIFO_tBuffer_T *InterfaceData);
void FifoBufferReceiveDataPop(FIFO_tBuffer_T *InterfaceData);
void FifoBufferSendDataPush(FIFO_tBuffer_T *InterfaceData);
void FifoBufferReceiveDataPush(FIFO_tBuffer_T *InterfaceData);
...
void FifoBufferInit(void)
{
    memset(&UART_InputBuffer,0,sizeof(fifoBuf_t));
    UART_InputBuffer.u16Size = FIFO_QUQUE_SIZE;

    #if defined (USE_UART_TX)
    memset(&UART_OutputBuffer,0,sizeof(fifoBuf_t));
    UART_OutputBuffer.u16Size = FIFO_QUQUE_SIZE;  
    #endif  
}
...
void FifoBufferSendDataPop(FIFO_tBuffer_T *InterfaceData)
{
    ...
    #if 1
    if ( !CheckFifoBufferEmpty(&UART_OutputBuffer)) //TX data
    {    
        FifoBufferPop(&UART_OutputBuffer, InterfaceData);

        // printf("UART Tx (Pop) : \r\n");
        // FifoBufferDump(InterfaceData,sizeof(FIFO_tBuffer_T));
    }
    #else
    FIFO_tBuffer_T t_Data; 

    if ( !CheckFifoBufferEmpty(&UART_OutputBuffer)) //TX data
    { 
        memset(&t_Data,0,sizeof(FIFO_tBuffer_T));    
        FifoBufferPop(&UART_OutputBuffer, &t_Data);

        printf("UART Tx (Pop) : \r\n");
        FifoBufferDump(t_Data,sizeof(FIFO_tBuffer_T));
    }
    #endif
...
{
    #if 1
    if ( !CheckFifoBufferEmpty(&UART_InputBuffer)) //RX data
    {   
        FifoBufferPop(&UART_InputBuffer, InterfaceData);
        
        // printf("UART Rx (Pop) : \r\n");
        // FifoBufferDump(InterfaceData,sizeof(FIFO_tBuffer_T));
    }
    #else
    FIFO_tBuffer_T r_Data; 

    if ( !CheckFifoBufferEmpty(&UART_InputBuffer)) //TX data
    { 
        memset(&r_Data,0,sizeof(FIFO_tBuffer_T));    
        FifoBufferPop(&UART_InputBuffer, &r_Data);
        
        printf("UART Rx (Pop) : \r\n");
        FifoBufferDump(r_Data,sizeof(FIFO_tBuffer_T));
    }
    #endif
}

void FifoBufferSendDataPush(FIFO_tBuffer_T *InterfaceData)    // output buffer
{    
    #if defined (USE_UART_TX)
    if ( !FifoBufferPush(&UART_OutputBuffer, InterfaceData) )
    {
        // printf("UART Tx (Push) : \r\n");
        // FifoBufferDump(InterfaceData,sizeof(FIFO_tBuffer_T));
    }
...
void FifoBufferReceiveDataPush(FIFO_tBuffer_T *InterfaceData) // input buffer
{
    if ( !FifoBufferPush(&UART_InputBuffer, InterfaceData) )
    {
        // printf("UART Rx (Push) : \r\n");
        // FifoBufferDump(InterfaceData,sizeof(FIFO_tBuffer_T));
    }
...
void UART13_IRQHandler(void)
{
    ...
    if(UART_GET_INT_FLAG(UART1, UART_INTSTS_RDAINT_Msk | UART_INTSTS_RXTOINT_Msk))     /* UART receive data available flag */
    {
        while(UART_GET_RX_EMPTY(UART1) == 0)
        {
			res = UART_READ(UART1);
            FifoBufferReceiveDataPush( (FIFO_tBuffer_T*)  &res);
        }
    }
